...
const ProfessorTopNav: React.FC<ProfessorTopNavProps> = ({ onLogout }) => {
  ...
  const [profile, setProfile] = React.useState<ProfessorProfile>({
    name: 'Encadrant',
    email: '',
    avatar: fallbackAvatar('Encadrant'),
  });
  ...
  React.useEffect(() => {
    ...
    const init = async () => {
      ...
      if (userData) {
        const name = `${userData.prenom || ''} ${userData.nom || ''}`.trim() || 'Encadrant';
        setProfile({
          name,
  ...
  const handleNotificationClick = async (notification: Notification) => {
    ...
    if (notification.type === 'MEETING_REQUEST' && notification.projet_id) {
      window.open(`https://meet.jit.si/PFEspace_Project_${notification.projet_id}`, '_blank');
    }
  };
          ...
          <div className={`flex items-center gap-3 ${compact ? 'justify-center' : 'justify-start'}`}>
            ...
            {!compact && (
              <div className="min-w-0">
                ...
                <p className="truncate text-[10px] font-medium text-[#64748B]">Encadrant</p>
              </div>
            )}
          </div>
        </div>
