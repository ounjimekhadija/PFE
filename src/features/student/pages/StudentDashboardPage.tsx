...
const StudentDashboard: React.FC = () => {
  ...
  if (loading) {
    return <div className="flex flex-1 items-center justify-center bg-[#F8FAFC] text-sm font-medium text-[#64748B]" style={{ fontFamily: 'Plus Jakarta Sans, system-ui, sans-serif' }}>Loading data...</div>;
  }
  ...
  return (
    <div
      className="flex h-full flex-col overflow-hidden bg-[#F8FAFC] px-4 py-3 text-[#1a1c1a]"
    ...
    >
      <header className="mb-3 flex items-end justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-[#64748B]">Welcome back! Here is what is happening in your current iteration.</p>
        </div>
        {data?.project?.deadline_globale && (
          <div className={`flex items-center gap-3 rounded-2xl border px-4 py-1.5 ${daysRemaining > 0 ? 'border-transparent bg-[#DCEBFA]/20' : 'border-transparent bg-[#ffdad6]'}`}>
            <div className="text-right">
              <p className={`text-[10px] font-bold uppercase tracking-widest ${daysRemaining > 0 ? 'text-[#1E3A5F]' : 'text-[#ba1a1a]'}`}>
                {daysRemaining > 0 ? "Project Ends In" : "Project Status"}
              </p>
              <p className={`text-base font-bold ${daysRemaining > 0 ? 'text-[#172D49]' : 'text-[#ba1a1a]'}`}>
                {daysRemaining > 0 ? `${daysRemaining} Days` : "Ended"}
              </p>
            </div>
            <Clock className={daysRemaining > 0 ? "text-[#172D49]" : "text-[#ba1a1a]"} size={18} />
          </div>
        )}
      </header>
      ...
      <div className="flex flex-col gap-5">
            ...
            <div className="mb-6 h-full rounded-2xl border border-transparent bg-white p-5 shadow-[0_4px_16px_rgba(15,23,42,0.06)]">
              ...
              <div className="relative h-28 flex items-center justify-center">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-widest text-[#64748B]">Done</span>
                </div>
              </div>
          ...
          >
            ...
            <div className="space-y-2.5 overflow-y-auto custom-scrollbar pr-1 w-full flex-1">
              {recentCompletedTasks.length === 0 ? (
                <p className="text-xs text-[#64748B]">No tasks completed recently.</p>
              ) : (
          ...
          >
            <div className="mb-4 shrink-0">
              ...
              <div className="flex flex-wrap gap-1.5 text-[9px] font-bold uppercase tracking-wider">
                ...
                <span className="rounded-md bg-[#fff4cc] px-2 py-1 text-[#1E3A5F]">Pending {pendingDeliverables}</span>
        ...
        <div className="rounded-2xl border border-[#C8D6E5]/60 bg-white p-4 shadow-[0_4px_16px_rgba(15,23,42,0.06)]">
          <div className="mb-3 flex items-center justify-between">
            <p className="text-xs font-bold uppercase tracking-widest text-[#64748B]">Chronology</p>
